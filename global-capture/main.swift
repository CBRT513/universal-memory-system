#!/usr/bin/env swift
//
//  Universal AI Memory - Global Capture Service (FIXED)
//  macOS System-Wide Memory Capture with Smart Intelligence
//  Fixed version that doesn't interfere with keyboard focus
//
...
class GlobalCaptureService: NSObject, NSMenuDelegate {
    ...
    // Use Carbon's proper hotkey API instead of event monitors
    private var hotkeyEventHandler: EventHandlerRef?
    private var hotkeyID = EventHotKeyID(signature: OSType("UMCS".fourCharCodeValue), id: 1)
    ...
    override init() {
        ...
        // Register proper Carbon hotkey that won't interfere
        ...
        print("✅ Using Carbon HotKey API - No keyboard interference")
        print("────────────────────────────────────────")
    }
    ...
    private func forceEnableMenuItemsHotfix() {
        ...
        // Register Carbon hotkey that won't interfere
        print("🚨 HOTFIX: Registering Carbon hotkey")
    ...
    private func registerCarbonHotkey() {
        ...
        if let existingHotkey = hotkeyRef {
            UnregisterEventHotKey(existingHotkey)
            hotkeyRef = nil
        }
        
        // Register ⌘⇧M using Carbon API (non-interfering)
        ...
        if status == noErr {
            hotkeyRef = hotkey
            print("✅ Carbon hotkey registered successfully (⌘⇧M)")
            ...
            InstallEventHandler(
                ...
                { (_, event, userData) -> OSStatus in
                    // Handle hotkey press
                    if let service = Unmanaged<GlobalCaptureService>.fromOpaque(userData!).takeUnretainedValue() as GlobalCaptureService? {
                        DispatchQueue.main.async {
                            service.handleGlobalHotkey()
                        }
                    }
                    return noErr
                },
    ...
    private func enableAllFeatures() {
        registerCarbonHotkey()  // Use Carbon API instead of event monitors
    ...
    private func setupStatusBarMenu() {
        ...
        let implementationQueueItem = NSMenuItem(title: "🚀 Implementations Needing Approval", action: #selector(showImplementationQueue), keyEquivalent: "i")
        implementationQueueItem.target = self
        implementationQueueItem.isEnabled = true
        menu.addItem(implementationQueueItem)
    ...
    private func showCaptureDialog() {
        ...
        // Try to get selected text first by simulating Cmd+C
        if let selectedText = getSelectedText(), selectedText.count > 0 {
            // Got selected text, show article capture dialog
            showCaptureDialogWithContent(selectedText, context: context)
        } else if let clipboardText = NSPasteboard.general.string(forType: .string), 
                  !clipboardText.isEmpty {
            // No selection, but clipboard has content - ask user
            showClipboardOrManualDialog(clipboardText, context: context)
        } else {
            // No selection and no clipboard - show quick capture options
            showQuickCaptureDialog(context: context)
        }
    }
    
    private func showClipboardOrManualDialog(_ clipboardText: String, context: AppContext) {
        let alert = NSAlert()
        alert.messageText = "📰 Capture Article"
        alert.informativeText = "No text selected. You can use clipboard content or write manually."
        alert.addButton(withTitle: "📋 Use Clipboard Content")
    ...
    private func showCaptureDialogWithContent(_ content: String, context: AppContext) {
        ...
        // Add article checkbox - positioned at top of controls
        let articleCheckbox = NSButton(checkboxWithTitle: "📰 This is an article", target: nil, action: nil)
        articleCheckbox.frame = NSRect(x: 0, y: 290, width: 200, height: 20)
        articleCheckbox.state = .on // Default to checked for article capture
        
        // Add article type dropdown - visible when article is checked
        let articleTypeLabel = NSTextField(labelWithString: "Article Type:")
        articleTypeLabel.frame = NSRect(x: 20, y: 265, width: 80, height: 17)
        
        let articleTypeDropdown = NSPopUpButton(frame: NSRect(x: 100, y: 263, width: 300, height: 22))
        articleTypeDropdown.addItem(withTitle: "🚀 Actionable - Needs implementation")
        articleTypeDropdown.addItem(withTitle: "📚 Reference - Keep for documentation")
        articleTypeDropdown.addItem(withTitle: "🎓 Learning - Educational content")
        articleTypeDropdown.addItem(withTitle: "🔍 Research - Needs further investigation")
        
        // Add regular category dropdown (for non-articles)
        let categoryLabel = NSTextField(labelWithString: "Category:")
        categoryLabel.frame = NSRect(x: 20, y: 265, width: 80, height: 17)
        categoryLabel.isHidden = true
        
        let categoryDropdown = NSPopUpButton(frame: NSRect(x: 100, y: 263, width: 300, height: 22))
        categoryDropdown.addItem(withTitle: "💡 Insight")
        categoryDropdown.addItem(withTitle: "🔧 Solution")
        categoryDropdown.addItem(withTitle: "💻 Code")
        categoryDropdown.addItem(withTitle: "🎨 Pattern")
        categoryDropdown.addItem(withTitle: "⚙️ Configuration")
        categoryDropdown.addItem(withTitle: "📚 Documentation")
        categoryDropdown.addItem(withTitle: "🐛 Debugging")
        categoryDropdown.addItem(withTitle: "⚡ Performance")
        categoryDropdown.isHidden = true
        
        // Add tags field
        let tagsField = NSTextField(frame: NSRect(x: 0, y: 230, width: 400, height: 22))
        tagsField.placeholderString = "Tags (comma-separated, e.g., swift, macos, memory)"
        
        // Add importance slider
        let importanceLabel = NSTextField(labelWithString: "Importance: 5")
        importanceLabel.frame = NSRect(x: 0, y: 200, width: 100, height: 17)
        
        let importanceSlider = NSSlider(frame: NSRect(x: 100, y: 200, width: 300, height: 20))
        ...
        // Container view - increased height to fit all elements
        let containerView = NSView(frame: NSRect(x: 0, y: 0, width: 400, height: 320))
        containerView.addSubview(scrollView)
        containerView.addSubview(articleCheckbox)
        containerView.addSubview(articleTypeLabel)
        containerView.addSubview(articleTypeDropdown)
        containerView.addSubview(categoryLabel)
        containerView.addSubview(categoryDropdown)
        ...
        articleCheckbox.target = self
        articleCheckbox.action = #selector(articleCheckboxToggled(_:))
        articleCheckbox.sendAction(articleCheckbox.action, to: articleCheckbox.target)
        
        alert.accessoryView = containerView
        
        let response = alert.runModal()
        if response == .alertFirstButtonReturn {
            var tags = tagsField.stringValue.split(separator: ",").map { String($0.trimmingCharacters(in: .whitespaces)) }
            
            var category = "note"
            if articleCheckbox.state == .on {
                category = "article"
                
                // Add article type tags based on selection
                let selectedIndex = articleTypeDropdown.indexOfSelectedItem
                switch selectedIndex {
                case 0: // Action Required
                    tags.append("actionable")
                    tags.append("implement-now")
                case 1: // Reference
                    tags.append("reference")
                    tags.append("keep-for-later")
                case 2: // Learning
                ...
                let categoryMap = ["Insight": "insight", "Solution": "solution", "Code": "code",
                                 "Pattern": "pattern", "Configuration": "configuration",
                                 "Documentation": "documentation", "Debugging": "debugging",
                                 "Performance": "performance"]
                let selectedTitle = categoryDropdown.titleOfSelectedItem ?? "Insight"
                let cleanTitle = selectedTitle.replacingOccurrences(of: "[^A-Za-z]", with: "", options: .regularExpression)
                category = categoryMap[cleanTitle] ?? "note"
            }
    ...
    @objc private func articleCheckboxToggled(_ sender: NSButton) {
        guard let containerView = sender.superview else { return }
        
        // Find the UI elements
        let articleTypeLabel = containerView.subviews.first { $0 is NSTextField && ($0 as! NSTextField).stringValue == "Article Type:" } as? NSTextField
        let articleTypeDropdown = containerView.subviews.first { view in
            if let popup = view as? NSPopUpButton {
                return popup.itemTitles.first?.contains("Action Required") ?? false
            }
            return false
        } as? NSPopUpButton
        
        let categoryLabel = containerView.subviews.first { $0 is NSTextField && ($0 as! NSTextField).stringValue == "Category:" } as? NSTextField
        let categoryDropdown = containerView.subviews.first { view in
            if let popup = view as? NSPopUpButton {
                return popup.itemTitles.first?.contains("Insight") ?? false
            }
            return false
        } as? NSPopUpButton
        
        if sender.state == .on {
            // Show article options, hide category
            articleTypeLabel?.isHidden = false
            articleTypeDropdown?.isHidden = false
            categoryLabel?.isHidden = true
            categoryDropdown?.isHidden = true
        } else {
            // Hide article options, show category
            articleTypeLabel?.isHidden = true
            articleTypeDropdown?.isHidden = true
    ...
    private func getSelectedText() -> String? {
        ...
        // Clear clipboard to ensure we detect new content
        ...
        if let copied = copiedText, !copied.isEmpty {
            // We got selected text!
            print("✅ Got selected text: \(copied.prefix(50))...")
            ...
            // No selection - restore original clipboard
    ...
    private func captureContent(_ content: String, source: String, context: AppContext, customTags: [String] = [], importance: Int = 5, category: String? = nil) {
        let allTags = generateTags(for: content, context: context) + customTags
        
        let memoryData: [String: Any] = [
            "content": content,
            "category": category ?? categorizeContent(content, context: context),
    ...
    @objc private func showImplementationQueue() {
        print("📋 Opening Implementation Queue...")
        
        Task {
            await MainActor.run {
                let window = ImplementationQueueWindow()
                window.show()
            }
        }
    }
