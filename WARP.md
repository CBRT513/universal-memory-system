...
### Overview
A comprehensive collection of 17 specialized Claude Code SubAgents that automate specific coding tasks, integrated with the Universal Memory System for pattern storage and context retrieval.
...
# Available agents: frontend-developer, backend-developer, api-developer, 
# mobile-developer, python-developer, javascript-developer, typescript-developer,
# devops-engineer, database-architect, cloud-architect, security-engineer,
# performance-engineer, test-automation-engineer, data-engineer, ml-engineer,
# code-reviewer, documentation-writer
```

### Usage Examples
```bash
# Create all subagent files for Claude Code
python3 subagents/cli/subagent_cli.py create-all

# Execute a specific subagent (requires Claude Code CLI)
...
python3 subagents/cli/subagent_cli.py show python-developer
```

### Memory Integration
- **Pattern Storage**: Successful solutions automatically stored in memory system
- **Context Retrieval**: Relevant past solutions retrieved for new tasks
- **Execution Logging**: All subagent runs logged with results and metadata
...
make workflow-fullstack     # Run full-stack development workflow
make workflow-deploy        # Run deployment preparation workflow

# Manual workflow chaining
python3 subagents/cli/subagent_cli.py execute frontend-developer "Create components"
python3 subagents/cli/subagent_cli.py execute backend-developer "Create API"
python3 subagents/cli/subagent_cli.py execute test-automation-engineer "Create tests"
```

### VS Code Integration
SubAgents are integrated with VS Code tasks in `.vscode/tasks.json`:
1. Open Command Palette (`Cmd+Shift+P`)
2. Run "Tasks: Run Task"
3. Select a SubAgent task (Frontend, Backend, Code Review, etc.)

### Configuration
- **Model Selection**: `subagents/configs/models.json` - Configure which models to use
- **Performance**: `subagents/configs/performance.json` - Caching, timeouts, resource limits
- **Templates**: `subagents/templates/` - YAML templates for each subagent

### Architecture
```
subagents/
├── templates/          # 17 SubAgent YAML templates
├── configs/           # Model and performance configuration
├── lib/              # Core management libraries
├── cli/              # Command-line interface
├── output/           # Generated Claude Code subagent files
└── workflows/        # Automated workflow scripts
```

### Creating Custom SubAgents
```bash
# Create custom subagent from instructions file
